...
@section('content')
    <div class="w-full mt-6 mx-auto">
        ...
        <a href="{{ route('peta.admin') }}"
            ...
            <form action="{{ route('peta.store') }}" method="POST" enctype="multipart/form-data" class="mt-6">
                @csrf
                ...
                {{-- Submit Button --}}
                <button type="submit" class="text-blue-600  bg-blue-300 hover:bg-blue-400 focus:ring-4 focus:outline-none focus:ring-indigo-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-indigo-500 dark:hover:bg-indigo-600 dark:focus:ring-indigo-700">Konfirmasi</button>
            </form>
        </div>
    </div>
@endsection
